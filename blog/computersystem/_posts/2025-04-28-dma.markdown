...
| 모드 | 특징 | 사용 예시 |
|:---:|:---:|:---:|
| Single/Cycle-Steal | 메모리 1~2 바이트(또는 word)마다 버스 반납 → CPU와 공존 | 오래된 ISA 카드, 저속 UART |
| Block/Burst | N 바이트 연속 전송 후 버스 반납. 캐시라인 단위 버스트가 일반적 | SATA, SDIO, 대부분의 PCIe 장치 |
| Demand/Fly-by | 장치 내부 FIFO가 찰 때까지만 전송, 실시간 스트림용 | 오디오 DAC, 카메라 센서 |
| Scatter-Gather | 링크드-리스트(Descriptor)로 여러 메모리 조각을 한 번에 전송 | NVMe, 고속 NIC, GPU DMA 복사 |
